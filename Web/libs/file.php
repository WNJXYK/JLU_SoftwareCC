<?php
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST'){
		if (isset($_POST["id"])){
			...
			if (move_uploaded_file($_FILES["file"]["tmp_name"], $_SERVER['DOCUMENT_ROOT'] . WEB_PATH . $path)){
				$database->insert("file", ["name" => $name, "path" => URL_PATH. WEB_PATH .$path, "size" => $siz, "user" => $uid]);
				exit(json_encode(["status"=>0, "data"=> ""]));
			}else exit(json_encode(["status"=>-1, "msg"=> "System Error"]));
		}
	}
